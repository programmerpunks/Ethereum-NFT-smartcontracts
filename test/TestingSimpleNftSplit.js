...
describe("Simple NFT with Reveal functionality", function () {
  ...
  async function deployContract() {
    ...
    const SimpleNFTSplit = await ethers.getContractFactory("simpleNFTSplit");
    const simpleNFTSplit = await SimpleNFTSplit.deploy(
      "Test",
      "test",
      "ipfs://URI/",
      "ipfs://notRevealedUri/",
    ...
    await simpleNFTSplit.setMintState(true);
    const accounts = [
      owner,
      account1,
      account2,
  ...
  describe("Constructor", () => {
    ...
    it("Should Revert when Accounts and percentages length mismatch ", async function () {
      await expect(loadFixture(deployContract4)).to.be.revertedWith(
        "Accounts and percentages length mismatch"
      );
    });
    it("Should Revert when partner must be at least equal to minPartners", async function () {
